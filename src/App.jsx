...
const calculateCBRFromScala = (scala) => {
    if (scala <= 0) return 0;
    return 318.15 * Math.pow(scala, -1.0788);
};

const calculateModifiedK = (k_subgrade, subbase_thickness) => {
...
const calculateEc = (fc) => fc > 0 ? 3320 * Math.sqrt(fc) + 6900 : 0;

// k1 factor from TM38 for load application time
const getK1 = (days) => days >= 90 ? 1.1 : 1.0;

// k2 factor from TM38 for load repetitions
const getK2 = (repetitionsStr) => {
...
const calculateAllowableStress = (fc, days, repetitions, postTensionStress = 0) => {
    if (!fc || fc <= 0) return 0;
    const baseFr = 0.456 * getK1(days) * getK2(repetitions) * Math.pow(fc, 0.66);
    return baseFr + parseFloat(postTensionStress);
};


const calculateL = (Ec, h, k) => {
...
// Aligned with TM38 Worked Examples & original XLSM File
const calculateSingleEdgeStress = (P_tonnes, h, l, b, jointType) => {
    if (h <= 0 || l <= 0 || b <= 0) return 0;
    const P_N = P_tonnes * 9810;
...
// Aligned with TM38 Worked Examples & original XLSM File
const calculateSingleCornerStress = (P_tonnes, h, l, r, jointType) => {
...
const getStressContribution = (distOverL, baseStress, type) => {
    let points;
...
const calculateTotalFactoredStress = (P_tonnes_unfactored, h, l, jointType, distances, loadCase, r_contact) => {
...
const generateStressGraphData = (l, P_tonnes, h, b, r, jointType) => {
...
const findMinThickness = (loadCase, allowableStress, Ec, k, P_tonnes_unfactored, jointType, distances, r_contact) => {
...
// --- UI Components ---
const NavButton = ({ onClick, children }) => (<button onClick={onClick} className="w-full flex justify-between items-center bg-indigo-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-indigo-700 transition duration-300">{children} <ChevronsRight /></button>);
const ReturnButton = ({ onClick }) => (<button onClick={onClick} className="flex items-center bg-gray-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-gray-700 transition duration-300"><ArrowLeft className="mr-2" /> Return to Input Parameters</button>);
const CustomTooltip = ({ active, payload, label }) => { if (active && payload && payload.length) { return (<div className="p-2 bg-gray-700 text-white rounded-md border border-gray-600 text-sm"><p className="label">{`Distance : ${label} mm`}</p>{payload.map((p, i) => ( <p key={i} style={{ color: p.color }}>{`${p.name} : ${p.value.toFixed(2)} MPa`}</p> ))}</div>); } return null; };
const StressChart = ({ data, title, lines }) => ( <div className="w-full h-64 mt-4"><h3 className="text-lg font-semibold text-center text-gray-700">{title}</h3><ResponsiveContainer><LineChart data={data} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="distance" label={{ value: 'Distance from load (mm)', position: 'insideBottom', offset: -5 }} /><YAxis label={{ value: 'Stress (MPa)', angle: -90, position: 'insideLeft' }} domain={['auto', 'auto']} /><Tooltip content={<CustomTooltip />} /><Legend />{lines.map(line => (<Line key={line.dataKey} type="monotone" dataKey={line.dataKey} name={line.name} stroke={line.color} strokeWidth={2} dot={false} />))}</LineChart></ResponsiveContainer></div>);

// --- Page Components ---

const InputParameters = ({ sharedInputs, setSharedInputs, setPage }) => {
    const handleInputChange = (e) => {
        const { name, value, type, checked } = e.target;
        setSharedInputs(prev => ({...prev, [name]: type === 'checkbox' ? checked : value }));
    };
    ...
    useEffect(() => {
        ...
        if (!isNaN(cbr) && cbr > 0) {
            ...
            setSharedInputs(prevInputs => ({
                ...prevInputs,
                subgradeModulus: k_subgrade.toFixed(2),
                finalModulus: finalModulus.toFixed(2)
            }));
        }
    }, [sharedInputs.stiffnessMethod, sharedInputs.cbrValue, sharedInputs.scalaValue, sharedInputs.useSubbase, sharedInputs.subbaseThickness]);
...
const PointLoadCalculator = ({ sharedInputs, setPage }) => {
...
const SingleRackCalculator = ({ sharedInputs, setPage }) => {
    ...
    const handleCalculateThickness = async () => {
        ...
        setTimeout(() => {
            const commonArgs = [designStrength, Ec, k, P_tonnes_unfactored, jointType];
            const interiorResult = findMinThickness('interior', ...commonArgs, singleRackDistances.interior, r_contact);
            setOutputs(prev => ({ ...prev, interior: { ...interiorResult, ultimateStrength: designStrength } }));
            const edgeResult = findMinThickness('edge', ...commonArgs, singleRackDistances.edge, r_contact);
             setOutputs(prev => ({ ...prev, edge: { ...edgeResult, ultimateStrength: designStrength } }));
            const cornerResult = findMinThickness('corner', ...commonArgs, singleRackDistances.corner, r_contact);
            setOutputs(prev => ({ ...prev, corner: { ...cornerResult, ultimateStrength: designStrength } }));
            setIsLoading(false);
        }, 100);
    };
    
    const criticalLoadCase = useMemo(() => {
        const results = Object.entries(outputs).filter(([_, val]) => val && val.thickness);
        if (results.length === 0) return null;
        let maxThickness = 0; 
        let criticalCaseName = null;
        results.forEach(([key, val]) => { 
            const thickness = parseFloat(val.thickness); 
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                <div className="lg:col-span-1">
                    <div className="bg-white p-4 rounded-lg shadow-md flex justify-center items-center border mb-4">
                        <img src="/single-rack.svg" alt="Single Line Rack Layout" className="max-w-full h-auto" />
                    </div>
                    <div className="bg-yellow-50 p-6 rounded-lg shadow-md border border-yellow-200">
...
const BackToBackRackCalculator = ({ sharedInputs, setPage }) => {
    ...
    const handleCalculateThickness = async () => {
        ...
        setTimeout(() => {
            ...
            setOutputs(prev => ({ ...prev, edgeLong: { ...edgeLongResult, ultimateStrength: designStrength } }));
            
            const edgeShortResult = findMinThickness('edgeShort', ...commonArgs, distances.edgeShort, r_contact);
            setOutputs(prev => ({ ...prev, edgeShort: { ...edgeShortResult, ultimateStrength: designStrength } }));
    ...
    return (
        <div className="p-4 md:p-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
             ...
             <div className="lg:col-span-2 space-y-6">
                 <div className="bg-white p-4 rounded-lg shadow-md flex justify-center items-center border"><img src="/backtoback-rack.svg" alt="Back-to-Back Rack Layout" className="max-w-xs" /></div>
                <div className="bg-blue-50 p-6 rounded-lg shadow-md border border-blue-200">
                         <h3 className="text-xl font-bold mb-4 text-gray-800 border-b pb-2">Outputs</h3>
...
const WheelLoadCalculator = ({ sharedInputs, setPage }) => {
    ...
    const handleCalculateThickness = async () => {
        ...
        setTimeout(() => {
            const commonArgs = [designStrength, Ec, k, P_wheel_tonnes_unfactored, jointType];
            const interiorResult = findMinThickness('interior', ...commonArgs, distances.interior, r_contact);
            setOutputs(prev => ({ ...prev, interior: { ...interiorResult, ultimateStrength: designStrength } }));
            
            const edgePerpResult = findMinThickness('edge', ...commonArgs, distances.edgePerp, r_contact);
            setOutputs(prev => ({ ...prev, edgePerp: { ...edgePerpResult, ultimateStrength: designStrength } }));
            
            const edgeParaResult = findMinThickness('edge', ...commonArgs, distances.edgePara, r_contact);
            setOutputs(prev => ({ ...prev, edgePara: { ...edgeParaResult, ultimateStrength: designStrength } }));
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                 <div className="bg-yellow-50 p-6 rounded-lg shadow-md border border-yellow-200">
                    ...
                    <div className="flex flex-col md:flex-row gap-4">
                        <img src="/wheel-loads.svg" alt="Wheel Loads" className="w-48 h-auto object-contain border bg-white p-2 rounded" />
                 ...
                 <div className="bg-blue-50 p-6 rounded-lg shadow-md border border-blue-200">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <ResultCard title="Interior" isLoading={isLoading} imgSrc="https://i.imgur.com/e4uC0H6.png" thickness={outputs.interior?.thickness} ultimateStrength={outputs.interior?.ultimateStrength?.toFixed(2)} factoredStress={outputs.interior?.stress?.toFixed(2)} isCritical={criticalLoadCase === 'interior'} />
                        <ResultCard title="Edge (Perpendicular)" isLoading={isLoading} imgSrc="https://i.imgur.com/sSg0s5M.png" thickness={outputs.edgePerp?.thickness} ultimateStrength={outputs.edgePerp?.ultimateStrength?.toFixed(2)} factoredStress={outputs.edgePerp?.stress?.toFixed(2)} isCritical={criticalLoadCase === 'edgePerp'} />
                        <ResultCard title="Edge (Parallel)" isLoading={isLoading} imgSrc="https://i.imgur.com/xIBo7YV.png" thickness={outputs.edgePara?.thickness} ultimateStrength={outputs.edgePara?.ultimateStrength?.toFixed(2)} factoredStress={outputs.edgePara?.stress?.toFixed(2)} isCritical={criticalLoadCase === 'edgePara'} />
                        <ResultCard title="Corner" isLoading={isLoading} imgSrc="https://i.imgur.com/a7YJ5k8.png" thickness={outputs.corner?.thickness} ultimateStrength={outputs.corner?.ultimateStrength?.toFixed(2)} factoredStress={outputs.corner?.stress?.toFixed(2)} isCritical={criticalLoadCase === 'corner'} />
                    </div>
                </div>
            </div>
        </div>
